...
"""Hub deploy service — item validation, source classification, deployment.

No ``click`` / ``rich_click`` imports.  Errors are raised as
:class:`~ewccli.services.exceptions.HubDeployServiceError`.
"""
...
HUB_ENV_VARIABLES_MAP: Dict[str, Dict[str, Optional[str]]] = {
...
class HubDeployService:
    """Pure business logic for hub item validation and deployment."""
    ...
    def categorize_item_inputs(
        ...
        hub_env_variables_map: Optional[dict] = None,
    ) -> Tuple[list, list]:
        """Categorize item inputs into default and mandatory.

        Returns ``(required_inputs, default_inputs)``.  The click
        context manipulation that the original function performed is
        left to the CLI wrapper.
        """
        if hub_env_variables_map is None:
            hub_env_variables_map = HUB_ENV_VARIABLES_MAP
    ...
    def validate_item_input_types(
        ...
        Returns "" if all inputs are valid, otherwise a string
        describing invalid inputs.
        """
    ...
    def classify_source(source: str) -> str:
        ...
        raise HubDeployServiceError(
            f"Source provided: {source} is not a valid GitHub repo URL or "
            "an absolute path to a local directory with content."
        )
    ...
    def check_github_repo_accessible(source: str) -> bool:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                _LOGGER.error(
                    f"Unexpected response ({response.status_code}) when checking: {source}"
                )
    ...
    def git_clone_item(
        ...
        if not HubDeployService.check_github_repo_accessible(source):
            _LOGGER.error("The repository is not accessible or does not exist.")
            raise HubDeployServiceError(
                f"Repository {source} is not accessible or does not exist."
            )
        ...
        _LOGGER.info(
            f"Starting to clone the repository '{repo_name}' into {command_path}..."
        )
        git_command = [
            f"""[ -d "$(basename '{source}' .git)/.git" ] || git clone '{source}' """
        ]
        return_code, message = ConfigService.run_command_from_host(
    ...
    def run_ansible_playbook_item(
        ...
        """Deploy Ansible item. Returns (return_code, message)."""
        ansible_return_code = HubDeployService.run_ansible_item(
            item=item,
